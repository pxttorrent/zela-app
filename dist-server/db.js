...
var query = function (text_1) {
    ...
    return __awaiter(void 0, __spreadArray([text_1], args_1, true), void 0, function (text, params) {
        var newUser, user, user, start, res, duration, err_1;
                ...
                case 0:
                    if (!connectionString) {
                        ...
                        if (text.includes('INSERT INTO users')) {
                            ...
                            return [2 /*return*/, { rows: [newUser], rowCount: 1 }];
                        }
                        // MOCK: Select User (Login or Check existence)
                        if (text.includes('SELECT') && text.includes('users')) {
                            // By ID (GetMe)
                            if (text.includes('id = $1')) {
                                user = mockUsers.find(function (u) { return u.id === params[0]; });
                                return [2 /*return*/, { rows: user ? [user] : [], rowCount: user ? 1 : 0 }];
                            }
                            // By Email (Login / Signup Check)
                            if (text.includes('email = $1')) {
                                user = mockUsers.find(function (u) { return u.email === params[0]; });
                                return [2 /*return*/, { rows: user ? [user] : [], rowCount: user ? 1 : 0 }];
                            }
                        }
